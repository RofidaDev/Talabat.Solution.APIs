...
namespace Talabat.Repository.Data
{
    public static class StoreContextSeed
    {
        ...
        {
            if (_dbContext.ProductBrands.Count() == 0)  //if table in DB is empty
            {
                var brandsData = File.ReadAllText("../Talabat.Repository/Data/DataSeeding/brands.json");


                var brands = JsonSerializer.Deserialize<List<ProductBrand>>(brandsData);  //from json to list
                if (brands?.Count() > 0)
                {
                    foreach (var brand in brands)
                    {
                        _dbContext.Set<ProductBrand>().Add(brand);  //in DB
                    }
            ...
            if (_dbContext.Products.Count() == 0)
            {
                ...
                //var productsData = File.ReadAllText("Data/DataSeeding/products.json");
